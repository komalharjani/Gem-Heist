...
divContainer.appendChild(table);
  ...
  disableAlarm: function() {
    this.makeMove();
      ...
      model.currState[currRow][currCol] = (false); //bind false to cell.className = "white";
      //add move to replay
      ...
      } if (currRow + 1 < model.height) {
          downCell = model.pieces[currRow + 1][currCol];
          alarmCheck.push(downCell);
      } if (currCol < model.width) {
      ...
      for (let i = 0; i < gemsFound.length; i++) {
          ...
          let alarmUp = (model.pieces[gemRow - 1][gemCol]);
          let alarmLeft = (model.pieces[gemRow][gemCol - 1]);
          //let alarmRight = (model.pieces[gemRow + 1][gemCol]);
          //let alarmDown = (model.pieces[gemRow + 1][gemCol]);
