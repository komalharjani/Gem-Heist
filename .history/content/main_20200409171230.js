...
const controller = {
  ...
  for (var i = 0; i < height; i++) {
    ...
    for (var j = 0; j < width; j++) {
        controller.currState[i].push(true); //default all true
    }
}
...
for (var i = 0; i < height; i++) { //loop through height
    ...
    for (var j = 0; j < width; j++) { //loop through width
        ...
        controller.pieces[i].push(cell);
        cell.setAttribute("row", i);
        cell.setAttribute("col", j);
        if(controller.currState[i][j] == true) { 
...
divContainer.appendChild(table);
  ...
  disableAlarm: function() {
    this.makeMove();
      ...
      controller.currState[currRow][currCol] = (false); //bind false to cell.className = "white";
      //add move to replay

      if (currRow - 1 >= 0) {
          upCell = controller.pieces[currRow - 1][currCol];
          alarmsAroundEvent.push(upCell);
      } if (currCol - 1 >= 0) {
          leftCell = controller.pieces[currRow][currCol - 1];
          alarmsAroundEvent.push(leftCell);
      } if (currRow + 1 < height) {
          downCell = controller.pieces[currRow + 1][currCol];
