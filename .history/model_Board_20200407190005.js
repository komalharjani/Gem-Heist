...
function captureAlarm() {
    ...
    if(currRow-1 >= 0) {
        ...
        alarmCheck.push(upCell);
    } else if(currRow+1 < height) {
        let downCell = pieces[currRow+1][currCol];
        alarmCheck.push(downCell);
    } else if(currCol-1 >= 0) {
        let leftCell = pieces[currRow][currCol - 1];
        alarmCheck.push(leftCell);
    } else if(currCol+1 < width) {
        let rightCell = [currRow][currCol+1];
